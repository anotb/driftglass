...
Wrangler provisions D1, R2, three Queues, a dedicated OAuth KV namespace for that environment, the `MissionComputer` Durable Object, Workflows, static assets, and Cron. Direct public-page reading works on Workers Free. Browser Rendering, Email Routing, AI Search, OpenAlex, the Companion, Computer Power Mode, and Agent Memory are optional setup after deployment.
...
- **Browser Rendering:** when a Mission needs Kitesurf or Chromium fallback, add the Cloudflare Browser binding after installation. The Driftglass Free profile stays within Cloudflare's current Browser Free allowance, and direct HTTP reading remains available without the binding.
