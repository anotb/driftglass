...
![Driftglass keeping an AI infrastructure answer connected to its supporting evidence](public/showcase/03-final-answer.png)

Try the frozen, read-only [live example](https://driftglass.unsol.dev/) or watch a standing question move from new sources to a saved answer in the [quick walkthrough](public/showcase/walkthrough.mp4).

## Product tour

| Today | Mission workspace |
|---|---|
| ![Three standing questions in the revised Driftglass Today view](public/showcase/01-today.png) | ![The AI infrastructure Mission workspace with its current files](public/showcase/02-mission-computer.png) |

| Public answer | Architecture |
|---|---|
| ![A cited public answer prepared by Driftglass](public/showcase/04-public-card.png) | ![Driftglass architecture from sources to a current answer](public/showcase/architecture.svg) |
...
This creates the Worker resources, applies D1 migrations, and sets 30-day expiry for disposable `raw/` captures. Direct public-page reading works on Workers Free. Browser Rendering, Email Routing, AI Search, the Companion, and Workers Paid are optional. See [Deployment](docs/DEPLOY.md) for costs, staging, and later upgrades.
...
Driftglass builds on and learns from open-source projects including OpenCLI and Cloudflare Computer. See [Acknowledgements](ACKNOWLEDGEMENTS.md) for the full list and each project’s role.
