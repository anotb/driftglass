...
export const REQUIRED_RELEASE_FILES = Object.freeze([
  ...
  "public/showcase/walkthrough.mp4",
  ...
  "public/icons/driftglass-lockup.png",
  "public/icons/driftglass-mark.png",
  "public/styles.css",
  "public/sw.js",
  "public/webmcp.js",
...
function sourceOnlyPaths(gitAttributes) {
  const exact = new Set();
  const directories = new Set();
  for (const line of String(gitAttributes).split(/\r?\n/)) {
    const match = line.trim().match(/^(\S+)\s+export-ignore(?:\s|$)/);
    if (!match) continue;
    let target = normalizedReleasePath(match[1]);
    if (target === ".internal" || target.startsWith(".internal/")) continue;
    if (target.endsWith("/**")) {
      target = target.slice(0, -3).replace(/\/+$/, "");
      if (target) directories.add(`${target}/`);
      continue;
    }
    if (/[*?[]/.test(target)) continue;
    exact.add(target);
  }
  for (const directory of directories) exact.delete(directory.slice(0, -1));
  return [...exact, ...directories].sort();
}
...
export function assertReleaseBoundaryProfile({ gitAttributes, gitIgnore, npmIgnore }) {
  ...
  const sourcePaths = sourceOnlyPaths(gitAttributes);
  return sourcePaths.length > 0 ? "source" : "public";
}
...
function assertPackageScriptReferencesResolve(packageManifest, archivedFiles) {
  const available = new Set(archivedFiles);
  const missing = [];
  for (const [name, command] of Object.entries(packageManifest.scripts ?? {})) {
    if (typeof command !== "string") continue;
    for (const match of command.matchAll(/\bnode(?:\.exe)?\s+("[^"]+"|'[^']+'|[^\s;&|]+)/g)) {
      const candidate = match[1].replace(/^(?:"([^"]+)"|'([^']+)')$/, "$1$2").replaceAll("\\", "/");
      if (!candidate.startsWith("scripts/") || available.has(candidate)) continue;
      missing.push(`${name} -> ${candidate}`);
    }
  }
  if (missing.length > 0) {
    throw new Error(`Tracked package.json references missing script files: ${[...new Set(missing)].sort().join(", ")}`);
  }
}
...
export function assertProviderBlockedDeployButtons(documents) {
  const active = documents
    .filter(([, source]) => deployButtonLinks(source).length > 0)
    .map(([label]) => label);
  if (active.length > 0) {
    throw new Error(
      `Tracked ${active.join(", ")} must not advertise Deploy to Cloudflare while that provider path is unavailable`,
    );
  }
...
export async function checkReleaseCandidate({
  ...
  try {
    ...
    assertPackageScriptReferencesResolve(archivedPackage, archivedFiles);
    const deployButtonState = assertProviderBlockedDeployButtons([
      ["README.md", await readFile(path.join(artifactRoot, "README.md"), "utf8")],
