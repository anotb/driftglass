...
import { frozenShowcaseEnabled, handleFrozenShowcaseRequest } from "./frozen-showcase";
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
      if (frozenShowcaseEnabled(env)) {
        return secureResponse(env, await handleFrozenShowcaseRequest(request, env), { assets: true });
      }
  ...
  async email(message: ForwardableEmailMessage, env: Env): Promise<void> {
    if (frozenShowcaseEnabled(env)) return;
    await ensureSchema(env.DB);
