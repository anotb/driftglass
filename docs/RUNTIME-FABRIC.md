...
capability, privacy, persistence, and budget requirements
        ↓
Worker → optional Kitesurf/Chromium → Mission Computer → Companion
```
...
The owner adds a source or starts a Routine. Direct HTTP is always available; the router chooses browser engines only when the optional Browser Rendering binding exists.
