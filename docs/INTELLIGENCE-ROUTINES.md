...
An Intelligence Routine is a bounded deterministic analyst playbook executed through Cloudflare Workflows.

It is not a prompt loop, agent swarm, or model router.
...
This keeps costs predictable and prevents the product from becoming pinned to one current model or provider API.
