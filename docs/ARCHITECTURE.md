...
Page collection starts with direct HTTP. When rendering is required, Driftglass tries Kitesurf before Chromium.
...
Private packet and MCP capabilities are bearer values carried in URL paths, so operational telemetry must not publish full request URLs. Release verification uses aggregate Workers Analytics, service-specific D1/Queue/R2/Workflow/Browser metrics, the Budget Governor ledger, and content-free readiness records. The public launch evidence contains no request logs. Source collection and Mission Computer code retains custom span boundaries so a future redacting telemetry adapter can use them without changing product behavior.
