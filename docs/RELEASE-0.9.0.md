...
![A finished Driftglass answer about LNG supply after Hormuz traffic resumed](assets/launch/03-final-answer.png)
...
The brief is compiled from saved state. The connected model does the interpretation and returns a cited answer through the v9 result contract.
...
Pack v3 can include sources, Missions, starter memory, source standards, reasoning instructions, output contracts, deterministic routines, budget expectations, and an update source.
...
The Cloudflare deployment is configured to provision the Worker, static assets, D1, R2, Queues, Workflows, Durable Object, Browser binding, Cron configuration, and a dedicated OAuth KV namespace. `npm run deploy` then applies D1 migrations and configures 30-day expiry for `raw/` captures.
