...
The Judgment Loop keeps the source state a model saw, the result it produced, the decision the user made, and what happened afterward.

## Evidence-State Receipt

Before a prepared analysis, Driftglass compiles and saves a receipt:
...
The D1 record retains the bundle hash, scope, provider target, task, objective, context-coverage rating, token estimate, truncation state, and R2 keys.
...
Only an approved change becomes durable memory.
