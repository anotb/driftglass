...
The one-click core already gives every Research Mission a durable `@cloudflare/computer` filesystem that works without a container or model API. This separately deployable Worker adds Cloudflare Computer's fast **Worker-shell** and **Worker-JavaScript** backends against the same agent-style workspace model.
