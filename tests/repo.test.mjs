...
test("project-owned packages use the repository MIT license", async () => {
  const [license, acknowledgements, rootPackage, rootLock, labPackage, labLock, memoryPackage, memoryLock, companionPackage] = await Promise.all([
    read("LICENSE"),
    read("ACKNOWLEDGEMENTS.md"),
    read("package.json"),
    read("package-lock.json"),
    read("labs/deep-dive-lab/package.json"),
    read("labs/deep-dive-lab/package-lock.json"),
    read("labs/agent-memory-bridge/package.json"),
    read("labs/agent-memory-bridge/package-lock.json"),
    read("driftglass-relay/package.json"),
  ]);
  assert.match(license.toString("utf8"), /^MIT License\n/);
  assert.doesNotMatch(acknowledgements.toString("utf8"), /no .* code is copied/i);
  for (const manifest of [rootPackage, labPackage, memoryPackage, companionPackage]) {
    assert.equal(JSON.parse(manifest.toString("utf8")).license, "MIT");
  }
  for (const lock of [rootLock, labLock, memoryLock]) {
    assert.equal(JSON.parse(lock.toString("utf8")).packages[""].license, "MIT");
  }
});

test("product showcase deploys as static files without capture metadata", async () => {
  const [page, manifestRaw] = await Promise.all([
    read("public/showcase/index.html"),
    read("public/showcase/manifest.json"),
  ]);
  const html = page.toString("utf8");
  const manifest = JSON.parse(manifestRaw.toString("utf8"));
  assert.match(html, /<video[\s\S]*walkthrough\.mp4/);
  assert.match(html, /href="hormuz-analysis\.html"/);
  assert.doesNotMatch(html, /captureFingerprint|approvalDigest|reviewedFrameCount/);
  assert.deepEqual(Object.keys(manifest), ["schemaVersion", "buildVersion", "illustrative", "artifacts"]);
  assert.ok(!("captureFingerprint" in manifest));
  assert.ok(!("modelInsert" in manifest));
  for (const entry of manifest.artifacts) {
    const asset = await read(`public/showcase/${entry.path}`);
    assert.equal(entry.bytes, asset.length, `${entry.path} byte count`);
    assert.equal(entry.sha256, createHash("sha256").update(asset).digest("hex"), `${entry.path} hash`);
  }
});
...
test("Cloudflare resources remain auto-provisionable", async () => {
  ...
  assert.equal(config.browser, undefined, "the clean-account default does not require Browser Run");
  assert.equal(config.env.staging.browser, undefined, "the public staging template remains one-click compatible");
...
test("Taste Profile, ranking explanations, and answer-led social preview cards close the learning and sharing loops", async () => {
  const [migration, taste, explain, mcp, webmcp, shares, publicSharePage, app, og, ogFingerprint, shareFallback, shareFallbackFingerprint, ogBuilder, attributes] = await Promise.all([
  ...
  assert.match(attributes.toString("utf8"), /^\*\.sha256 text eol=lf$/m);
