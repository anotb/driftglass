...
test("every deployment command preserves deploy, migration, and lifecycle order", async () => {
  ...
  assert.ok(
    npmignore.split(/\r?\n/).includes("/public/showcase/"),
    "product showcase media stays out of the runtime npm package",
  );
});

test("default deployment documentation includes the dedicated OAuth KV for each environment", async () => {
...
test("default deployment resources remain deterministic, unpinned, and failure-bounded", async () => {
  const config = JSON.parse(await read("wrangler.jsonc"));
...
test("release docs distinguish prerequisites, automatic provisioning, and optional setup", async () => {
  const [readme, deploy, install, roadmap] = await Promise.all([
    read("README.md"),
    read("docs/DEPLOY.md"),
    read("public/install.md"),
  ...
  assert.doesNotMatch(readme, /deploy\.workers\.cloudflare\.com/, "the provider-blocked deployment button stays inactive");
...
test("public docs do not advertise the provider-blocked one-click deployment path", async () => {
  const files = [
    "CONTRIBUTING.md",
    "lenses/README.md",
    "labs/deep-dive-lab/README.md",
    "labs/agent-memory-bridge/README.md",
  ];
  const documents = await Promise.all(files.map(async (file) => [file, await read(file)]));
  const deploymentClaim = /\bone-click\b[^\n]{0,80}\b(?:deploy(?:ment|ments)?|core)\b/i;
  assert.match("one-click Worker deployment", deploymentClaim);
  assert.match("one-click core", deploymentClaim);
  assert.doesNotMatch("one-click source validation", deploymentClaim);
  assert.doesNotMatch("one-click semantic retrieval setup", deploymentClaim);

  for (const [file, source] of documents) {
    assert.doesNotMatch(source, deploymentClaim, `${file} uses the supported source-checkout or default-deployment wording`);
  }
});
